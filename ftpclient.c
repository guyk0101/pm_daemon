...
static const char *ftpmsgenv = "PM_FTPMSG";
static int pm_ftpmsg = 0;
...
struct sockaddr local;
int ftp_connect(char *hostname, char *username, char *passwd, ftp_printf_t ftp_printf) {
...
int ftp_put_file(char *filename, char *buf, unsigned buf_size, int s, ftp_printf_t ftp_printf) {
    int ret;
    char msgbuf[256];
    int data_s;

    ftp_read_t ftp_read = _ftp_read;
    struct _ftp_data ftp_data;
    void *ftp_read_priv;
    ftp_data.buf = buf;
    ftp_data.len = buf_size;
    ftp_read_priv = &ftp_data;
...
int ftp_close(int s, ftp_printf_t ftp_printf) {
    int ret;
    char msgbuf[256];
